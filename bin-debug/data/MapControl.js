...
var MapControl = (function () {
    ...
    //在游戏初始时候，根据空行创建元素
    //在游戏过程中，根据数量创建元素
    MapControl.prototype.createElement = function (num) {
        var types = [];
        for (var i = 0; i < num; i++) {
    ...
    MapControl.prototype.updateMapLocation = function () {
        ...
        for (var i = 0; i < len; i++) {
            ...
            for (var t = 0; t < l; t++) {
                var rel = false; //创建一个变量，结果默认为false
                var ll = ids.length;
                for (var r = 0; r < ll; r++) {
        ...
        var colarr = []; //记录每个元素的列编号，因为元素的每次移动是向下移动
        for (var t = 0; t < len; t++) {
            rel = false;
            for (var i = 0; i < colarr.length; i++) {
                //有i代表取到了其中的列编号，我们需要重新计算它的行编号，因为下落都是从最顶部进行掉落
                if (colarr[i] == GameData.elements[ids[t]].location % GameData.MaxColumn) {
                    return true;
                }
            }
            if (!rel) {
        ...
        var coleids; //列进行元素规划，对整行列进行调整
        len = colarr.length; //列编号的长度
        for (var t = 0; t < len; t++) {
            var newcolids = []; //当前这个新列的ID
            var removeids = []; //移除的ID
    ...
    MapControl.prototype.createAllMap = function () {
        ...
        var type = "";
        var havelink = true; //是否存在连接
        var id = 0;
        var ztype = ""; //记录上方两个元素相同时的元素类型
        var htype = ""; //记录左侧两个元素相同时的元素类型
