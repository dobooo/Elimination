...
var Linklogic = (function () {
    ...
    Linklogic.isHaveLine = function () {
        ...
        var currentType = ""; //标记当前类型的存储器
        var typeNum = 0; //当前检索类型的数量
        //检索整个列
    ...
    Linklogic.canMove = function (id1, id2) {
        ...
        var l1col = GameData.elements[id1].location % GameData.MaxColumn; //对元素的实际编号进行求余
    ...
    Linklogic.chageOrder = function () {
        ...
        //编写随机数
        var index = 0; //作为暂存数据,存放随机出的具体数的一位数组的下标，之后用于存放到地图数据当中
        for (var i = 0; i < GameData.MaxRow; i++) {
            for (var t = 0; t < GameData.MaxColumn; t++) {
